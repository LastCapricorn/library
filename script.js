...
function showLibraryAsCardSet(lib) {
  ...
  if (lib.data !== []) {
    for (let book = 0; book < lib.data.length; book++) {
      ...
      card.setAttribute("data-title", lib.data[book].title)

      for (const key in lib.data[book]) {
        if (Object.hasOwnProperty.call(lib.data[book], key)) {
          ...
          if (key === "read") {
            card.appendChild(
              attachSwitch(lib.data[book][key], lib.data[book].title)
            )
          } else {
            cardText.textContent = lib.data[book][key]
      ...
      const removeButton = attachRemoveButton(lib.data[book].title)
      removeButton.addEventListener("click", () => {
        lib.removeFromLibrary(removeButton.value)
...
function showLibraryAsTable(lib) {
  ...
  const switchInput = document.querySelectorAll(".switch-container .switch")
  switchInput.forEach((sw) =>
...
function initializeLibrary() {
  ...
  if (localStorage && localStorage.length !== 0) {
    ...
    const exampleLibrary = new Library("Example Library")
    setOptions.textContent = exampleLibrary.name
    setOptions.setAttribute("selected", "")
    setSelection.appendChild(setOptions)
    exampleLibrary.data = defaultLibrary
    exampleLibrary.exportLibrary()
    showLibraryAsCardSet(exampleLibrary)
  } else {
